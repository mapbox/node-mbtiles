...
require('util').inherits(MBTiles, EventEmitter)
function MBTiles(uri, callback) {
    ...
    // EventEmitter lock to prevent parallel commits.
    this._lock = new EventEmitter();
    this._lock.setMaxListeners(0);
...
MBTiles.prototype._commit = function(callback) {
    var mbtiles = this;
    if (mbtiles._committing) return mbtiles._lock.once('commit', callback);
    else mbtiles._committing = true;

    // Copy cached data into scope and then clear caches synchronously.
    // Ensures that data to be inserted is determined at call time rather
    // than allowing additional data to sneak in at callback steps.
    var q = {
        _tileCache: mbtiles._tileCache,
        _gridCache: mbtiles._gridCache,
        _dataCache: mbtiles._dataCache,
        _keyCache: mbtiles._keyCache,
        _mapCache: mbtiles._mapCache
    };
    mbtiles._clearCaches();

    Step(function() {
        mbtiles._db.run('BEGIN', this);
    }, function(err) {
        if (err) throw err;
        if (!Object.keys(q._tileCache).length) return this();
        // Insert images table.
        var images = mbtiles._db.prepare('REPLACE INTO images (tile_id, tile_data) VALUES (?, ?)');
        for (var id in q._tileCache) images.run(id, q._tileCache[id]);
        images.finalize(this);
    }, function(err) {
        if (err) throw err;
        if (!Object.keys(q._gridCache).length) return this();
        // Insert grid_utfgrid table.
        var grids = mbtiles._db.prepare('REPLACE INTO grid_utfgrid (grid_id, grid_utfgrid) VALUES (?, ?)');
        for (var id in q._gridCache) grids.run(id, q._gridCache[id]);
        grids.finalize(this);
    }, function(err) {
        if (err) throw err;
        if (!Object.keys(q._keyCache).length) return this();
        // Insert grid_key.
        var keys = mbtiles._db.prepare('INSERT OR IGNORE INTO grid_key (grid_id, key_name) VALUES (?, ?)');
        for (var id in q._keyCache) {
            for (var i in q._keyCache[id]) {
                keys.run(id, q._keyCache[id][i]);
            }
        }
        keys.finalize(this);
    }, function(err) {
        if (err) throw err;
        if (!Object.keys(q._dataCache).length) return this();
        // Insert keymap table.
        var keymap = mbtiles._db.prepare('REPLACE INTO keymap (key_name, key_json) VALUES (?, ?)');
        for (var key in q._dataCache) {
            keymap.run(key, JSON.stringify(q._dataCache[key]));
        }
        keymap.finalize(this);
    }, function(err) {
        if (err) throw err;
        ...
        if (!Object.keys(q._mapCache).length) return this();
        ...
        var mapGrid = mbtiles._db.prepare('REPLACE INTO map (zoom_level, ' +
            ...
            'AND tile_column = ? AND tile_row = ?), ?)');
        for (var coords in q._mapCache) {
            var map = q._mapCache[coords];
        ...
        mapBoth.finalize(this.parallel());
        mapTile.finalize(this.parallel());
        mapGrid.finalize(this.parallel());
    }, function(err) {
        if (err) throw err;
        mbtiles._db.run('COMMIT', this);
    }, function(err) {
        mbtiles._committing = false;
        mbtiles._lock.emit('commit', err);
        callback(err);
    });
};
