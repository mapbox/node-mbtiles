...
/**
 * Get a tilestats object from the mbtiles metadata table
 *
 * @param {Function} callback - includes an `err` object first, and a `stats` object second
 * @returns {Object} tilestats - a tilestats object as defined in mapbox-geostats
 *
 */
MBTiles.prototype.getTilestats = function(callback) {
    this._db.all('SELECT value FROM metadata WHERE name = \'tilestats\' LIMIT 1', function(err, row) {
        if (err && err.errno !== 1) return callback(err);
        if (!row.length) return callback(null, null); // if tilestats row doesn't exist, don't error but return null
        try { var tilestats = JSON.parse(row[0].value); }
        catch (err) { return callback(new Error('Tilestats object is invalid JSON: ' + err.message)); }
        return callback(null, tilestats);
    });
};
