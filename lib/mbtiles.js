    ...
    this._db.get(sql, z, x, y, function(err, row) {
        if ((!err && !row) || (err && err.errno == 1)) {
            ...
            mbtiles.getInfo(function(err, info) {
                ...
                var sniffedContentType = tiletype.contentType(row.tile_data);

                if (info.format === 'pbf' || sniffedContentType === 'application/x-protobuf') {
                    headers['Content-Type'] = 'application/x-protobuf';
                    zlib.inflate(row.tile_data, function(err, data) {
                        if (err) return callback(err);
                        return callback(null, data, headers);
                    });
                } else {
                    headers['Content-Type'] = sniffedContentType;
...
MBTiles.prototype.putTile = function(z, x, y, data, callback) {
    ...
    function write(data) {
        mbtiles.write('images', id, {
            tile_id: id,
            tile_data: data
        });
        mbtiles.write('map', coords, {
            zoom_level: z,
            tile_column: x,
            tile_row: y,
            tile_id: id
