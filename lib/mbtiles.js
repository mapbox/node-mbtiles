...
function MBTiles(uri, callback) {
    ...
    if (!uri.query.mode) uri.query.mode = 'READWRITE | CREATE';
    var flagEnum = {
        READONLY: sqlite3.OPEN_READONLY,
        READWRITE: sqlite3.OPEN_READWRITE,
        CREATE: sqlite3.OPEN_CREATE
    };
    var flags = uri.query.mode.split('|').map(function(flag) {
        return flag.trim();
    });
    var mode = flags.reduce(function(results, flag) {
        if (flagEnum[flag]) results |= flagEnum[flag];
        return results;
    }, 0);
    if (mode === 0) mode = sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE;
